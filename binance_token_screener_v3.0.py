...
performs multi-dimensional analysis, and generates comprehensive reports with Feishu integration.

Core Features:
- Feishu authentication for spreadsheet integration
...
class BinanceTokenScreenerV1:
    ...
    def init_feishu_connection(self):
        """初始化飞书连接"""
        print("🔐 初始化飞书连接...")

        try:
            # 初始化飞书管理器
            self.feishu_manager = FeishuManager(self.feishu_config_file)
            print("✅ 飞书连接成功")
            return True
        except FileNotFoundError:
            print("❌ 未找到飞书配置文件: feishu_config.json")
            print("📝 请创建配置文件并添加:")
    ...
    def authenticate(self):
        """认证入口函数（飞书版本）"""
        return self.init_feishu_connection()
    ...
    def format_data_for_sheets(self, df):
        """Format data for Feishu spreadsheet display"""
        ...
        try:
            ...
            if isinstance(files, str):
                ...
                if files.endswith('.xlsx'):
                    return self.upload_excel_file_to_feishu(files)
                else:
                    print("❌ 不支持的文件格式")
                    return False
            elif isinstance(files, list):
                # 多个CSV文件
                return self.upload_csv_files_to_feishu(files)
            else:
                print("❌ 无效的文件参数")
                return False
    ...
    def upload_excel_file_to_feishu(self, excel_filename):
        """上传Excel文件到飞书表格"""
        try:
            # 读取Excel文件
    ...
    def upload_csv_files_to_feishu(self, csv_files):
        """上传CSV文件到飞书表格"""
        try:
            worksheets_data = {}
            ...
            for csv_file in csv_files:
                if os.path.exists(csv_file):
                    ...
                    sheet_name = self.worksheets_config.get(base_name, base_name)
                    worksheets_data[sheet_name] = df
                    print(f"  📋 准备工作表: {sheet_name}")
    ...
    def display_results(self, datasets, enhanced_df, data_files=None, offline=False):
        ...
        if not offline and hasattr(self, 'sheet') and self.sheet:
            ...
            print("💡 离线模式提示: 如需上传到飞书表格，请稍后在网络正常时重新运行")
